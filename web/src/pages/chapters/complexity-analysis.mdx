...
import { binarySearchTrace } from '../../lib/traces';
...
about 20 units of work, a linear one does a million, and a quadratic one does a trillion. At the
billion-or-so simple operations a second one core manages, that is the difference between *instant*
and *about a quarter of an hour* — and one more zero on `n` stretches that quarter hour past a full
day. That is why growth rate is the first thing to reason about, and why the rest of this book
cares about it.
...
That is genuinely most of the notation. Before we push on, it is worth *seeing* where O(log n)
comes from, because it is the one class that feels like magic. Binary search throws away half of
what remains at every step — so a thousand elements take ten comparisons, a million take twenty.
Watch the search window collapse:

<AlgoViz client:visible kind="binarySearch" steps={binarySearchTrace} title="Watch it run: binary search halves the space" />

The interesting part — the part most books skip — is
...
- `std::sort` is `O(n log n)` and beats a hand-written `O(n log n)` mergesort on in-memory data,
  because its partition loop is in-place and cache-resident while the merge pays for a scratch
  buffer on every level. Same complexity class, and the constant decides it.
