...
import { treeSearch, treeInorder } from '../../lib/snippets';
...
~100ns — roughly 300 cycles — fetching a line from main memory, and the *only* useful thing on
that line is one
...
- **Linux process scheduling** — the Completely Fair Scheduler (CFS), the default from 2007 until EEVDF replaced its core algorithm in Linux 6.6 (2023), keeps runnable tasks in a red-black tree keyed by each task's virtual runtime. "Who runs next" is just "leftmost node," and inserting a woken task is O(log n). Ordered structure, chosen for a hot kernel path.
