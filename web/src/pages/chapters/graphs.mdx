...
slug: graphs
description: Traversal, shortest paths, A*, flow, and the representation choices that dominate runtime.
---
import AlgoViz from '../../components/problem/AlgoViz';
import { bfsTrace, dfsTrace, dijkstraTrace } from '../../lib/traces';
...
## Representations: The Memory Decision That Governs Everything
...
Watch DFS dive as deep as it can down one branch and only backtrack at a dead end — the **stack** (not a queue) is exactly what makes it go deep before wide:

<AlgoViz client:visible kind="graph" steps={dfsTrace} title="Watch it run: DFS goes deep, then backtracks" />
...
You just read BFS in four languages; here it is on a small graph. Watch the frontier expand outward one level at a time — the queue drives it, and every vertex is reached by the shortest number of edges from the source:

<AlgoViz client:visible kind="graph" steps={bfsTrace} title="Watch it run: BFS explores level by level" />
...
Watch Dijkstra always pull the closest unfinalized node off the **priority queue**, relax its outgoing edges, and lower its neighbors' tentative distances. The greedy "closest first" order is what the priority queue buys you:

<AlgoViz client:visible kind="graph" steps={dijkstraTrace} title="Watch it run: Dijkstra, closest node first" />
...
A **minimum spanning tree (MST)** is the cheapest set of edges that keeps every vertex connected — think of laying cable to wire up every building for the least total length. Both classic algorithms are greedy ([Chapter 18](/chapters/greedy-algorithms)), and both are correct for the same reason: the *cut property* guarantees the lightest edge crossing any partition of the vertices is safe to include.
...
The root needs its own rule, because it has no ancestor to escape to and the low-link test is meaningless there. Instead, the root is an articulation point precisely when it has two or more DFS children: two independent subtrees joined only through it. (Every bridge forces an articulation point at whichever of its ends has another edge — in a lone two-vertex component neither end qualifies — but not the reverse: a vertex can be critical without any single edge being.)
...
## Concurrency: Traversing a Shared Graph

Parallelizing graph work runs straight into the concurrency fundamentals of [Chapter 4](/chapters/concurrency-fundamentals), and the sharpest hazard is the `visited` array. Two threads that both read `visited[v] == false` will both process `v` — the classic check-then-act race — corrupting results or looping forever. The fix is to make the check-and-set a single atomic step:
