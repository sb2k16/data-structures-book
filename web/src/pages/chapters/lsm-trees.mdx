...
import { lsmTrace } from '../../lib/traces-systems';
...
Follow a batch of writes through the whole path — into the sorted memtable, flushed to L0 as
immutable runs, then compacted into a single larger run in L1:

<AlgoViz client:visible kind="lsm" steps={lsmTrace} title="Watch it run: memtable → flush → compaction" />
...
<InProduction>
- **RocksDB and LevelDB** — the canonical embedded LSM engines. RocksDB (Facebook's fork of Google's LevelDB) is the storage layer inside an enormous amount of infrastructure: it backs Kafka Streams' state stores, TiKV, and countless internal databases (CockroachDB ran on it too, until it moved to Pebble, its own LSM engine, in 2021).
...
into one logical database is consistent hashing. That is how systems like Cassandra, ScyllaDB, and
