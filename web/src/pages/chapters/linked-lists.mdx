...
import { reverseTrace } from '../../lib/traces';
...
prefetcher can't help, and each hop is very likely a **cache miss**: a ~100ns stall, roughly 300
cycles, waiting for main memory. This is the **pointer chase**, and you pay it once per node, for the entire length of
the list.
...
just careful pointer bookkeeping. Fill in the function and run the tests.

<CodeProblem client:visible problem={reverseList} />

<AlgoViz client:visible kind="list" steps={reverseTrace} title="Watch it run: reversing the pointers" />
