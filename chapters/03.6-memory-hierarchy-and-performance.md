...
void benchmarkAccessPatterns() {
    ...
    auto sequential_time = duration_cast<microseconds>(end - start).count();
    ...
    auto random_time = duration_cast<microseconds>(end - start).count();
    
    cout << "Sequential access: " << sequential_time << " microseconds" << endl;
    cout << "Random access: " << random_time << " microseconds" << endl;
    cout << "Speedup: " << (double)random_time / sequential_time << "x slower" << endl;
    cout << "Sums match: " << (sum1 == sum2 ? "Yes" : "No") << endl;
}
```
...
```cpp
void benchmarkCacheEffects() {
    const int MAX_SIZE = 32 * 1024 * 1024;  // 32 MB
    vector<int> data(MAX_SIZE / sizeof(int));
    ...
    vector<int> sizes = {1024, 4096, 16384, 65536, 262144, 1048576, 4194304};
    ...
    for (int size : sizes) {
        ...
        double cycles_per_element = (double)time / size * 2.5;  // Assume 2.5 GHz
        cout << "Size: " << size << " elements, Time: " << time << " ns, "
             << "Cycles/element: " << cycles_per_element << endl;
    }
}
```
