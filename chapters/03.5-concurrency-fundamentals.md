...
flowchart TD
    ...
    ReadOnly -->|Yes| SharedMutex["Use shared_mutex<br/>(read-write lock)"]
...
### Implementation with std::shared_mutex
...
**Reader Locks (`std::shared_lock`)**:
- Multiple threads can hold shared locks simultaneously
- Blocks writers from acquiring exclusive lock
- Does not block other readers

**Writer Locks (`std::unique_lock`)**:
- Only one thread can hold an exclusive lock
- Blocks all readers and other writers
- Ensures exclusive access for modifications

**Lock Acquisition Rules**:
1. **Readers**: Can acquire lock if no writer holds it
2. **Writers**: Can acquire lock only if no readers or writers hold it
3. **Priority**: Implementation-dependent (see starvation discussion below)
...
void reader(ThreadSafeDictionary& dict, int id) {
    for (int i = 0; i < 10; i++) {
        int value = dict.lookup("key" + std::to_string(i % 5));
        std::cout << "Reader " << id << " read: " << value << std::endl;
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
    }
}

void writer(ThreadSafeDictionary& dict, int id) {
    for (int i = 0; i < 5; i++) {
        dict.insert("key" + std::to_string(i), id * 100 + i);
        std::cout << "Writer " << id << " wrote key" << i << std::endl;
...
int main() {
    ...
    for (int i = 0; i < 5; i++) {
        readers.emplace_back(reader, std::ref(dict), i);
    }
    
    // Create writers
    std::vector<std::thread> writers;
    for (int i = 0; i < 2; i++) {
        writers.emplace_back(writer, std::ref(dict), i);
    }
...
**Core Invariants**:
1. **Multiple Readers**: Any number of readers can hold shared locks simultaneously
2. **Exclusive Writer**: Only one writer can hold an exclusive lock at a time
3. **Mutual Exclusion**: Readers and writers cannot access simultaneously
4. **Data Consistency**: Writers see a consistent view when they acquire the lock
...
- Readers seeing intermediate states of write operations
...
2. **No Writer Priority**: Standard `std::shared_mutex` doesn't prioritize writers
...
void writer(int id) {
    while (true) {
        std::unique_lock<std::shared_mutex> lock(rw_mutex);  // May wait forever!
        shared_data++;
...
int main() {
    ...
    // because readers continuously acquire shared locks
}
```
...
private:
    ...
    std::mutex writer_mutex;  // Serialize writer requests
    
public:
    void read_lock() {
        reader_semaphore.acquire();  // Wait for reader slot
    ...
    void write_lock() {
        writer_mutex.lock();  // Ensure only one writer waits at a time
        rw_mutex.lock();  // Exclusive lock
    }
...
**Don't use when**:
- Writes are frequent (overhead of reader-writer lock may not be worth it)
- You need guaranteed writer progress (use regular mutex or custom solution)
- Read operations are very fast (mutex overhead may be negligible)
- You need strict fairness guarantees

### Comparison: Mutex vs Shared Mutex

| Aspect | `std::mutex` | `std::shared_mutex` |
|--------|--------------|---------------------|
...
## 3.5.13 What This Book Will and Will Not Do
