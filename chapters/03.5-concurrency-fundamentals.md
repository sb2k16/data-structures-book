...
**Core Invariants**:
1. **Multiple Readers**: `readers > 0` and `writer == false` means multiple readers can access
2. **Exclusive Writer**: `writer == true` means only one writer, and `readers == 0`
3. **Mutual Exclusion**: `writer == true` implies `readers == 0`
4. **No Starvation Prevention**: This implementation is reader-preferring (see below)

**What Must Not Be Observed**:
- Partial writes while readers are reading
- Multiple writers active simultaneously
