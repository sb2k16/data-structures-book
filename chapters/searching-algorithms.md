...
```python
# Search the inclusive range [lo, hi]; lo/hi default to the whole array.
def binary_search(a, target, lo=None, hi=None):
    if lo is None:
        lo, hi = 0, len(a) - 1
    while lo <= hi:
...
**The overflow.** Writing `mid = (lo + hi) / 2` is the natural, obvious, and wrong way to find the midpoint. When `lo` and `hi` are both large, `lo + hi` overflows a signed 32-bit `int` and becomes negative; `a[mid]` then indexes out of bounds and you get a crash or garbage. This is not a hypothetical — it is one of the most famous bugs in the field. It sat undetected in the Java standard library's binary search for nine years — and in Jon Bentley's *Programming Pearls*, the book that popularized the algorithm, for two decades before that — until someone hit an array big enough to trigger it. The fix is to compute the offset from `lo` instead: `lo + (hi - lo) / 2` can never overflow, because `hi - lo` is smaller than `hi`. Write it that way every time.
...
```python
# First index i with a[i] >= target.  (This is bisect.bisect_left.)
...
# First index i with a[i] > target.  (This is bisect.bisect_right.)
...
def jump_search(a, target):
    ...
    prev = 0
    while a[min(step, n) - 1] < target:    # find the block that may hold target
        prev = step
        step += math.isqrt(n)
        if prev >= n:
            return -1
    for i in range(prev, min(step, n)):    # linear scan within the block
        if a[i] == target:
            return i
    return -1
```
...
def interpolation_search(a, target):
    ...
    while lo <= hi and a[lo] <= target <= a[hi]:
        if a[lo] == a[hi]:                       # flat range: no interpolation
            return lo if a[lo] == target else -1  # (also guards divide-by-zero)
